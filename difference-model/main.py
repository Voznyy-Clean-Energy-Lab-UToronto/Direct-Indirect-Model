...
parser.add_argument('-b', '--batch-size', default=216, type=int,
                    metavar='N', help='mini-batch size (default: 256)')
parser.add_argument('--lr', '--learning-rate', default=0.00001, type=float,
                    metavar='LR', help='initial learning rate (default: '
                                       '0.01)')
parser.add_argument('--lr-milestones', default=[1000], nargs='+', type=int,
                    metavar='N', help='milestones for scheduler (default: '
                                      '[100])')
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--weight-decay', '--wd', default=0.00000001, type=float,
...
parser.add_argument('--atom-fea-len', default=126, type=int, metavar='N',
...
def main():
    ...
    validate(val_loader, model, criterion, normalizer, test=False, val=True)
    validate(train_loader, model, criterion, normalizer, test=False, val=False, train=True)
...
def validate(val_loader, model, criterion, normalizer, test=False, val=False, train=False):
    batch_time = AverageMeter()
    losses = AverageMeter()
    if args.task == 'regression':
        mae_errors = AverageMeter()
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        if args.task == 'regression':
            ...
            if val:
                ...
                val_cif_ids += batch_cif_ids
            if train:
                train_pred = torch.exp(output.data.cpu())
                train_target = target
                assert train_pred.shape[1] == 2
                train_preds += train_pred[:, 1].tolist()
                train_targets += train_target.view(-1).tolist()
                train_cif_ids += batch_cif_ids
    ...
    if test:
        ...
        with open('val_results.csv', 'w') as f:
            writer = csv.writer(f)
            for cif_id, target, pred in zip(val_cif_ids, val_targets,
                                            val_preds):
                writer.writerow((cif_id, target, pred))
    elif train:
        star_label = '**'
        ...
        with open('train_results.csv', 'w') as f:
            writer = csv.writer(f)
            for cif_id, target, pred in zip(train_cif_ids, train_targets,
                                            train_preds):
                writer.writerow((cif_id, target, pred))
    else:
        star_label = '*'
